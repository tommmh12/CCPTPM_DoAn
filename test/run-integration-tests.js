...
async function main() {
  try {
    ...
    await runStep("login rejects malformed input with validation details", async () => {
      await resetTestUserState();
    ...
    await runStep("task lifecycle routes create, update, star, and delete a task", async () => {
      await resetTestUserState();
      ...
      const starResponse = await fetch(`${baseUrl}/api/tasks/${taskId}/star`, {
        method: "PATCH",
        headers,
        body: JSON.stringify({
          isStarred: true
      ...
      assert.equal(detailResult.payload.task.title, "Integration task updated");
